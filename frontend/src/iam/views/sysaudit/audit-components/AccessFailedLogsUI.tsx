...
const AccessFailedLogsUI = () => {
  ...
  return (
    <div className={`flex flex-col space-y-2`}>
      ...
      />
      <div
        className={`pt-3 mt-3 px-5 w-[60%] h-full overflow-y-scroll flex flex-col space-y-5`}
      >
        {failedGroupLogs?.map((failedLogs, index) => {
          const apiDate = new Date(failedLogs?.date);
          const checkDateWithToday = isToday(apiDate)
            ? "Today"
            : format(apiDate, "EEE dd MMM yyyy");
            ...
            <div key={index} className={`flex flex-col`}>
              <div className={`flex justify-between items-center`}>
                <div
                  ...
                  key={failedLogs.date}
                  onClick={() => accordionToggleHandler(failedLogs.date)}
                >
                  <BiIcons.BiSolidCaretDownCircle size={20} />
                  <div className={`font-Poppins text-[12px]`}>
                    {checkDateWithToday}
                  </div>
                </div>
                <div className={`font-Poppins text-[12px]`}>
                  {failedLogs.logs.length || 0} records
                </div>
              </div>
              {isActive(failedLogs.date) && (
                <div
                  className={`py-5 ${
                    failedLogs.logs.length > 3
                      ? "h-[250px] overflow-y-scroll border"
                      : "h-full"
                  }`}
                >
                  {failedLogs?.logs?.map((log, index) => (
                    <div
                      key={index}
                      className={`flex space-x-4 px-10 relative  ${
                    ...
                    >
                      ...
                      <div className="flex flex-col items-center">
                        ...
                        {index !== failedLogs.logs.length - 1 && (
                          <div className="w-[2px] h-24 bg-gray-100 mt-1 rounded-full" />
                        )}
                      </div>
                      ...
                      <div className={`flex-grow  my-1 `}>
                        <div className={`flex justify-between pt-4 px-5`}>
                          <div className={`flex flex-col space-y-2`}>
                            <div
                              className={`text-[13px] flex space-x-2 items-center`}
                            >
                              ...
                              <span
                                className={`text-[#333] text-opacity-50 text-[12px] `}
                              >
                                Authtenication failed due to
                                <strong className={`text-red-600 pl-2`}>
                                  {log.failure_reason}
                                </strong>
                              </span>
                            </div>
