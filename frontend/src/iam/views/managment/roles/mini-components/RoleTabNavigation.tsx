...
const RoleTabNavigation = () => {
    ...
    return ( 
      // ${activeTabIndex === index ? "border-b-[2px] border-primary-green bg-gray-50 rounded-t-[5px] text-text-primary pt-[14px] pb-1.5 duration-500 transition ease-in-out": "pt-5 pb-1"}
        ...
        <FlexOuterContainer className='relative w-full font-Poppins text-[14px] mt-5 '>
          <FlexBox className='flex justify-between items-center border-b py-3 mx-1'>
            <FlexBoxInner className='flex justify-start space-x-2 px-3 flex-grow pt-3 '>
              {
                  role_tab?.map((roleTab, index) => (
                      <div key={index} onClick={() => changeTabIndex(index)}  className={`relative cursor-pointer rounded-[5px]  py-[6px] px-3 bg-gray-50 ${activeTabIndex === index && 'border-b-[2px] border-black'}`}>
                             {
                              roleTab.total && (
