...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        document_appproval = ApprovalProcessModel.objects.create(
            process_name="Document Approval",
            active=True,
            allow_recall = True
        )

        # Create states 
        saved = WorkflowStateModel.objects.create(state_name='saved', state_description="approved by owner state", state_in_protocol=document_appproval)
